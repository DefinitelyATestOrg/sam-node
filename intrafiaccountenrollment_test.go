...
func TestIntrafiAccountEnrollmentNew(t *testing.T) {
	...
	_, err := client.Intrafi.AccountEnrollments.New(context.TODO(), samgo.IntrafiAccountEnrollmentNewParams{
		AccountID:    samgo.F("account_in71c4amph0vgo2qllky"),
		EmailAddress: samgo.F("user@example.com"),
	})
	if err != nil {
...
func TestIntrafiAccountEnrollmentListWithOptionalParams(t *testing.T) {
	...
	_, err := client.Intrafi.AccountEnrollments.List(context.TODO(), samgo.IntrafiAccountEnrollmentListParams{
		...
		Status: samgo.F(samgo.IntrafiAccountEnrollmentListParamsStatus{
			In: samgo.F([]samgo.IntrafiAccountEnrollmentListParamsStatusIn{samgo.IntrafiAccountEnrollmentListParamsStatusInPendingEnrolling, samgo.IntrafiAccountEnrollmentListParamsStatusInEnrolled, samgo.IntrafiAccountEnrollmentListParamsStatusInPendingUnenrolling}),
		}),
	})
	if err != nil {
		var apierr *samgo.Error
