...
import { Sam, ClientOptions } from 'sam';
...
function getTSDiagnostics(code: string): string[] {
  ...
  const codeWithImport = [
    ...
    functionSource.type === 'declaration' ?
      `async function run(${functionSource.client}: Sam)`
    : `const run: (${functionSource.client}: Sam) => Promise<unknown> =`,
    functionSource.code,
  ].join('\n');
...
const fuse = new Fuse(
  [
    ...
    'client.messages.countTokensBeta',
