...
export interface ClientOptions {
   ...
   * Set the log level.
   *
   * Defaults to process.env['SAM_LOG'] or 'warn' if it isn't set.
   */
...
export class Sam {
   ...
   */
  constructor({
    baseURL = readEnv('SAM_BASE_URL'),
    apiKey = readEnv('API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.SamError(
        "The API_KEY environment variable is missing or empty; either provide it, or instantiate the Sam client with an apiKey option, like new Sam({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? Sam.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('SAM_LOG'), "process.env['SAM_LOG']", this) ??
      defaultLogLevel;
