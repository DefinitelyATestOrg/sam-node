...
func TestExternalAccountNewWithOptionalParams(t *testing.T) {
	...
	_, err := client.ExternalAccounts.New(context.TODO(), samgo.ExternalAccountNewParams{
		AccountNumber: samgo.F("987654321"),
		Description:   samgo.F("Landlord"),
		RoutingNumber: samgo.F("101050001"),
		AccountHolder: samgo.F(samgo.ExternalAccountNewParamsAccountHolderBusiness),
		Funding:       samgo.F(samgo.ExternalAccountNewParamsFundingChecking),
	})
	if err != nil {
...
func TestExternalAccountUpdateWithOptionalParams(t *testing.T) {
	...
	_, err := client.ExternalAccounts.Update(
		...
		samgo.ExternalAccountUpdateParams{
			AccountHolder: samgo.F(samgo.ExternalAccountUpdateParamsAccountHolderBusiness),
			Description:   samgo.F("New description"),
			Funding:       samgo.F(samgo.ExternalAccountUpdateParamsFundingChecking),
			Status:        samgo.F(samgo.ExternalAccountUpdateParamsStatusActive),
		},
	)
	if err != nil {
		var apierr *samgo.Error
...
func TestExternalAccountListWithOptionalParams(t *testing.T) {
	...
	_, err := client.ExternalAccounts.List(context.TODO(), samgo.ExternalAccountListParams{
		...
		Status: samgo.F(samgo.ExternalAccountListParamsStatus{
			In: samgo.F([]samgo.ExternalAccountListParamsStatusIn{samgo.ExternalAccountListParamsStatusInActive, samgo.ExternalAccountListParamsStatusInArchived}),
		}),
	})
	if err != nil {
		var apierr *samgo.Error
