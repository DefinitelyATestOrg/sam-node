...
async function main() {
  ...
  const accountRetrieveResponse: Sam.Customers.AccountRetrieveResponse =
    await sam.customers.accounts.retrieve('REPLACE_ME', 'REPLACE_M', params);
}
...
async function main() {
  const accountRetrieveResponse = await sam.customers.accounts
    .retrieve('REPLACE_ME', 'REPLACE_M', { userId: '36a22460-ebc8-4ffe-a213-1683c5a420c5' })
    .catch(async (err) => {
...
await sam.customers.accounts.retrieve('REPLACE_ME', 'REPLACE_M', { userId: '36a22460-ebc8-4ffe-a213-1683c5a420c5' }, {
  maxRetries: 5,
});
```
