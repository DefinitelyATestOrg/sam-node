...
func TestLockboxNewWithOptionalParams(t *testing.T) {
	...
	_, err := client.Lockboxes.New(context.TODO(), samgo.LockboxNewParams{
		AccountID:   samgo.F("account_in71c4amph0vgo2qllky"),
		Description: samgo.F("Rent payments"),
	})
	if err != nil {
...
func TestLockboxUpdateWithOptionalParams(t *testing.T) {
	...
	_, err := client.Lockboxes.Update(
		...
		samgo.LockboxUpdateParams{
			Description: samgo.F("x"),
			Status:      samgo.F(samgo.LockboxUpdateParamsStatusInactive),
		},
	)
	if err != nil {
		var apierr *samgo.Error
